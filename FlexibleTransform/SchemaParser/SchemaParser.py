...
class SchemaParser(object) :
    ...
    * self.logging - The logging object
        ...
        for rowType in rowTypes :
            if (rowType in MappedData) :
                if (isinstance(MappedData[rowType],list)) :
                    ...
                    self.TransformedData[rowType] =  self._TransformDataToNewSchema(rowType, MappedData[rowType], None, DocumentMetaData, oracle)
                else :
                    raise Exception('NoParsableDataFound', "Data isn't in a parsable dictionary format")
            else :
                self.TransformedData[rowType] = self._TransformDataToNewSchema(rowType, None, None, DocumentMetaData, oracle)
                ...
                if ('defaultValue' in fieldDict and fieldDict['defaultValue'].startswith('&')) :
                    ...
                    if (match) :
                        ...
                        if (args and args in self.SchemaConfig[rowType]['fields']) :
                            if (field in FieldOrder and ( args not in FieldOrder or FieldOrder[args] >= FieldOrder[field] ) ) :
                        ...
                        for m in match :
                            if (m[1] != '') :
                                if (m[1] in self.SchemaConfig[rowType]['fields']) :
                                    if (field in FieldOrder and ( m[1] not in FieldOrder or FieldOrder[m[1]] >= FieldOrder[field] ) ) :
        ...
        for field in FieldOrder :
            ...
            if (mappedField is not None) :
                ...
                if (isinstance(Value,list) and 
                    ...
                    if (fieldDict['datatype'] == 'group') :
                            ...
                            if (isinstance(row,dict)) :
                                ...
                                for (subkey, subvalue) in subRow.items() :
                                    if (subkey in ValueMap and ValueMap[subkey] not in subfields) :
                                        raise Exception('FieldNotAllowed','Field %s is not an allowed subfield of %s' % (ValueMap[subkey], field))
                                ...
                                raise Exception('DataError', 'Data type of sub row for %s is not dict: %s' % (mappedField, row))
                        ...
                        for d in Value :
                            if (isinstance(d,(list,dict))) :
                                self.logging.warning('%s subvalue in the list is another list or dictionary, not currently supported: %s', mappedField, d)
                                continue
                    ...
                    self.logging.warning('%s value is a list or dictionary, not currently supported: %s', mappedField, Value)
                    continue
                    ...
                    if ('requiredIfReferenceValues' in fieldDict) :
                        ...
                        for val in ReferenceValues :
                            if ( 
                                ...
                                ( val == '' and (not ReferenceField in newDict or not newDict[ReferenceField]['Value']) ) ) :
                                required = True
                                break
                ...
                if (required == True or ('required' in fieldDict and fieldDict['required'] == True) ) :
                    if ('defaultValue' in fieldDict) :
                        ...
                        if (newDict[field]['Value'].startswith('&')) :
                            newDict[field]['Value'] = self._CalculateFunctionValue(newDict[field]['Value'], field, newDict[field], rowType, newDict, IndicatorType=None, TransformedData=self.MappedData)                              
                        ...
                        if ('memberof' in fieldDict) :
                            self.logging.warning("Sub-groups should not have 'required' set to true, processing skipped: %s", field)
                        else :
                            groupRow = { 'fields': {} }
                            self._BuildFieldGroup(None, newDict, rowType, field, groupRow, None)
                            
                    else :
                        raise Exception('NoDefaultValue', 'Default Value or outputFormat not defined for required field %s' % field)
    
        if (not SubGroupedRow) :
            for field in DataRow :
                if (field not in processedFields) :
                    self.logging.warning('%s not processed for row type %s in schema config. Value: %s', field, rowType, DataRow[field])
    ...
    def _ValidateField(self, fieldDict, fieldName, rowType) :
        ...
        if ('Value' in fieldDict) :
            if (fieldDict['Value'].startswith('&')) :
                raise Exception('ValueIsFunction','Value for field %s maps to a function which should already have been processed: %s' % (fieldName, fieldDict['Value']))
            ...
            if ('ontologyMappingType' in fieldDict) :
                if (fieldDict['ontologyMappingType'] == 'none') :
                    ...
                    if (fieldDict['ontologyMapping'] != '') :
                        ...
                        if oracle is not None:
                            OntologyReferences.extend(oracle.getCompatibleConcepts(OntologyReference))
                        else:
                            if (OntologyReference in DataDictionary) :
                                OntologyReferences[OntologyReference].extend(DataDictionary[OntologyReference].keys())
                                # Use the ontology to determine a sub-group.
                                # What does self._BuildFieldGroup do/change?
                                    ...
                                    for reverseMapping in v['reverseOntologyMappings'] :
                                        if (reverseMapping in DataDictionary) :
                                            if (fieldDict['datatype'] == 'enum') :
                                                ...
                                                self.logging.warning('reverseOntologyMappings in field %s not supported because datatype is not enum', k)
                                            break
                ...
                raise Exception('MissingOntologyMappingType', 'The OntologyMappingType is missing from field %s' % field)
            ...
            if (len(OntologyReferences) == 0) :
                ...
                if (required == True or ('required' in fieldDict and fieldDict['required'] == True) ) :
                    if ('defaultValue' in fieldDict) :
                        ...
                        if (newDict[field]['Value'].startswith('&')) :
                            newDict[field]['Value'] = self._CalculateFunctionValue(newDict[field]['Value'], field, newDict[field], rowType, newDict, IndicatorType, TransformedData=self.TransformedData)
    ...
    def _BuildFieldGroup(self, DataDictionary, groupDict, rowType, group, groupRow, IndicatorType) :
                ...
                if (k not in groupRow['fields']) :
                    ...
                    if (isinstance(v,list)) :
                        for v2 in v :
                            ...
                            if (v2.startswith('&')) :
                                v2 = self._CalculateFunctionValue(v2, k, self.SchemaConfig[rowType]['fields'][k], rowType, groupDict, IndicatorType, TransformedData=self.TransformedData)
                        ...
                        if (v.startswith('&')) :
                            v = self._CalculateFunctionValue(v, k, self.SchemaConfig[rowType]['fields'][k], rowType, groupDict, IndicatorType, TransformedData=self.TransformedData)
        ...
        for fieldDict in groupRow['fields'][primaryKey] :
            ...
            if (fieldGroup[primaryKey]['Value'].startswith('&')) :
                fieldGroup[primaryKey]['Value'] = self._CalculateFunctionValue(fieldGroup[primaryKey]['Value'], primaryKey, fieldGroup, rowType, fieldGroup, IndicatorType, TransformedData=self.TransformedData)
            ...
            for requiredField in requiredFields :
                if (requiredField in groupRow['fields']) :
                    ...
                    # Determine if any of the defined fields match this primary key
                        ...
                        for k in groupRow['fields'][requiredField] :
                            if ('NewValue' in k and k['NewValue'] in self.SchemaConfig[rowType]['fields'][requiredField]['enumValues'] and 
                                'primaryKeyMatch' in self.SchemaConfig[rowType]['fields'][requiredField]['enumValues'][k['NewValue']]) :
                                
                                primaryKeyMatch = self.SchemaConfig[rowType]['fields'][requiredField]['enumValues'][k['NewValue']]['primaryKeyMatch']
                ...
                if (requiredField not in fieldGroup) :
                    ...
                    if ('defaultValue' in fieldGroup[requiredField]) :
                        ...
                        self._BuildFieldGroup(DataDictionary, fieldGroup, rowType, requiredField, groupRow, IndicatorType)
                        continue
                    ...
                    if (fieldGroup[requiredField]['Value'].startswith('&')) :
                        fieldGroup[requiredField]['Value'] = self._CalculateFunctionValue(fieldGroup[requiredField]['Value'], requiredField, fieldGroup, rowType, fieldGroup, IndicatorType, TransformedData=None)
    ...
    def _ConvertValueToTargetSchema(self, field, fieldDict, sourceDict, Value) :
        ...
        if (fieldDict['datatype'] == 'datetime') :
            if ('ParsedValue' in sourceDict) :
                ...
                self.logging.error('DateTime data type did not have a ParsedValue defined for field %s (%s)', field, fieldDict)
            ...
            if (fieldDict['datatype'] == 'string' or fieldDict['datatype'] == 'enum') :
                ...
                if (fieldDict['datatype'] == 'ipv4' and re.match(r'^([0-9]{1,3}\.){3}[0-9]{1,3}$', Value)) :
                    ...
                    self.logging.warning("Cannot convert between data types for field %s (%s, %s)", field, fieldDict['datatype'], sourceDict['datatype'])
    ...
    def _CalculateFunctionValue(self, value, fieldName, fieldDict, rowType, row=None, IndicatorType=None, TransformedData=None ):
        '''
        '''
        if (value.startswith('&')) :
            ...
            if (match) :
                ...
                if (FunctionScopeValid) :
                    ...
                    self.logging.warning('Function %s in field %s is not valid for current document scope %s', function, fieldName, rowType)
            else :
                raise Exception('InvalidFunctionFormat','The function reference for field %s, %s, is not valid' % (fieldName, value))
        else :
