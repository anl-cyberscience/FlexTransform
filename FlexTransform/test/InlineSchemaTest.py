...
class Test(unittest.TestCase):
    ...
    def setUp(self):
        logging.basicConfig(format='%(name)s (%(pathname)s:%(lineno)d) %(levelname)s:%(message)s', level=logging.CRITICAL)
    ...
    def test_inline_schema(self):
        ...
        FinalizedData = self.Transform.TransformFile(sourceFileName=sample_cfm13_file, targetFileName=target_csv_file, sourceParserName='Cfm13Alert', targetParserName='InlineSchema')
